...
namespace ModpackCalculator
{
    internal partial class ModManager
    {
        public void ExportAsCSV(string fileName = "output.csv") // optional value/overload to export all/certain conditions?
        {
            ...
            var newMods = Mods.Where(x => x.Status.HasFlags(ModStatus.CurrentMod, ModStatus.Installed) && !x.Status.HasFlag(ModStatus.PreviousMod)).OrderBy(x => x.ModName);
            var removedMods = Mods.Where(x => x.Status.HasFlag(ModStatus.PreviousMod) && !x.Status.HasFlag(ModStatus.CurrentMod)).OrderBy(x => x.ModName);
            var unmatchedMods = Mods.Where(x => x.Status.HasFlag(ModStatus.CurrentMod) && !x.Status.HasFlag(ModStatus.Installed)).OrderBy(x => x.ModName); //Unmatched Mods
